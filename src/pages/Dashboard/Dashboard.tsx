...
const Dashboard: React.FC = () => {
  ...
  const [tasks, setTasks] = useState<Task[]>(storedUser.todos?.map(normalizeTask) || []);
  const [inlineSelectedTask, setInlineSelectedTask] = useState<Task | null>(null);
  ...
  //   setInlineSelectedTask((prev) => (prev?.id === id ? { ...prev, status: newStatus } : prev));
  //   setSearchSelectedTask((prev) => (prev?.id === id ? { ...prev, status: newStatus } : prev));
  // };
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex flex-1 flex-col lg:flex-row">
        ...
        <main className="flex-1 mt-14 lg:mt-0 bg-[#F5F8FF] p-3 lg:p-0">
          <Outlet context={{ tasks, handleSaveTask, handleDelete, handleUpdateStatus, inlineSelectedTask, setInlineSelectedTask, isEditing, setIsEditing }} />
        </main>
      </div>
